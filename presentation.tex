...
\frame{
\frametitle{Better modeling tools have set the stage for \\ 
            optimization to play a larger role in design}
    \begin{columns}
    \begin{column}{.5\textwidth}
      Experiment
      \vspace{.5em}
      \includegraphics[width=\textwidth]{images/sim_airplane_real}
    \end{column}
    \begin{column}{.5\textwidth}
      Simulation
      \vspace{.5em}
      \includegraphics[width=\textwidth]{images/sim_airplane_cfd}
    \end{column}
    \end{columns}
}

\frame{
\frametitle{Large scale optimization is particularly challenging}
  \hspace{5em}\includegraphics[width=.7\textwidth]{images/opt_cycle_budget}
}

\frame{
\frametitle{Gradient based optimization is \\
            very efficient for large design spaces}
  \centering
  \includegraphics[width=.85\textwidth]{images/gradfree_v_gradbased}
}

\frame{
  \frametitle{Gradient based methods achieve super-linear\\ 
              scaling with increasing problem size}
  \centering
  \includegraphics[width=.7\textwidth]{images/opt_scaling}
}



\frame{
  \frametitle{But first you have to get get the gradient!}
  \centering
  \includegraphics[height=.75\textheight]{images/complexity-cartoon} 
}

\frame{
  \frametitle{Gradients are useful outside of optimization too}

  \LARGE\begin{itemize}[leftmargin=3.5em,itemsep=1em]
...
\frame{
  \frametitle{Model has a very complex sturcture  \\ 
              built with NREL WISDEM }
  \includegraphics[width=\textwidth]{images/wt_xdsm} 
}
...
\frame{
  ...
  \begin{columns}
      \begin{column}{.5\textwidth}
        \includegraphics[width=1.1\textwidth]{images/cadre_prob_form}
      \end{column}
      \begin{column}{.5\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/cadre_drawing}

        \hspace{3em}\includegraphics[width=.55\textwidth]{images/allpts_gearth_mcmurdo}
      \end{column}
  \end{columns}
  
  \begin{textblock}{8}(12,4)\scriptsize
    [Hwang, et al.  2014]
  \end{textblock}
}
...
\frame{
  \frametitle{Pymission finds optimial trajectories that \\
              match traditionally expected results}
  \includegraphics[width=\textwidth]{images/Scitech_CarpetRange}
}
...
\frame{
  \frametitle{Gradients are very useful, OpenMDAO computes them \\ 
               efficiently for multidisciplinary problems}
